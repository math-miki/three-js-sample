...
window.onload = function() {
  ...
  const pause_start_button = document.getElementById("b_p_s");
  const cange_color = document.getElementById("c_c_r");
  const cange_rotation = document.getElementById("c_r_r");
  let geometries = [];
  let cubes = [];
  let cubeParams = [];
  let mov = true;
  ...
  cange_color.onclick = function() {
    for(let i=0; i<cubes.length; i++) {
      c = new THREE.Color(0.35+Math.random()*0.5,0.35+Math.random()*0.5,0.35+Math.random()*0.5);
      cubes[i].material.color = c;
    }
  }
  cange_rotation.onclick = function() {
    for(let i=0; i<cubeParams.length; i++) {
      cubeParams[i] = [Math.random()*0.4-0.1,Math.random()*0.4-0.1];
    }
  }
